...
object Ex3Main extends App {
  ...
  val best10DaysDF: DataFrame = retailerDataFrame.orderBy("ORDERDATE").sort(desc("QUANTITYORDERED * PRICEEACH")).show(10)
  ...
  val sales2019: Sales = salesDS.filter($"year" = 2019)
  println(f"Sales for 2019 is ${sales2019.euros}")

  val maximumSales: Sales = salesDS.groupBy("year").filter(max("euros"))
  ...
  val multiSales2019: Sales = multiSalesDS.filter($"year" = 2019)
  println(f"Total sales for 2019 is ${multiSales2019.euros}")

  val maximumMultiSales: Sales = multiSalesDS.groupBy("year").filter(max("euros"))
  ...
  val bestDaysDFStreaming = retailerStreamingDF.orderBy("ORDERDATE").sort(desc("QUANTITYORDERED * PRICEEACH")).show(10)
  ...
  bestDaysDFStreaming.write.format("csv").mode("overwrite).save(outputPath/file.csv)
