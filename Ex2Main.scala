...
import org.apache.spark.sql.functions.{col, sum, min, max, asc, desc, udf, year}
...
object Ex2Main extends App {
  ...
  val weatherDataFrame: DataFrame = spark.read.csv("ex2/data/rdu-weather-history.csv")
  ...
  val minTemp: Double = weatherDataFrame.collect(min("temperaturemin"))
  val maxTemp: Double = weatherDataFrame.collect(max("temperaturemax"))
  ...
  val aggregatedDF: DataFrame = weatherDataFrameWithYear.groupBy("year").max("temperaturemax").min("temperaturemin")
  ...
  val task6DF: DataFrame = weatherDataFrameWithYear.groupBy("year").count().orderBy("avgwindspeed").count()
